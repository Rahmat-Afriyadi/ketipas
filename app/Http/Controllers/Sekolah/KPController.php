...
class KPController extends Controller{
    ...
    static function index(){
        $data['url']  = 'x';
        return view('sekolah.kp',$data);
    }

    static function GetKP($req){
        $id_user  = Auth::ID(); $success = false; $sekolah = ''; $import = false; $data = []; $message = 'Gagal Get Data';
        ...
        if($opr){
            ...
            if($ta){
                ...
                if($req->jenis == 1){
                    ...
                    foreach($data as $dat){
                        $tanggal  = DB::table('ta_kp')->where('nik',$dat->nik)->value('tanggal');
                        if(!$tanggal){
                            $tanggal = '-';
                        }
                        $datas[]  = [
    ...
    static function UpdateKP($url,$req){
        $exp  = explode('-',$url);
        $jenis = $exp[1];
        $success = false; $message = 'Gagal Update Data'; $tanggal = '-';
        ...
        $cek  = DB::table('ta_kp')->where('nik',$req->nik)->first();
        if($cek){
            DB::table('ta_kp')->where('nik',$req->nik)->update([
                'tanggal' => $tanggal,
                'jenis' => $jenis,
            ]);
            $success = true; $message = 'Sukses Update Data';
        }else{
            DB::table('ta_kp')->insert([
                'nama' => $req->nama,
                'nik' => $req->nik,
                'jenis' => $jenis,
                'tanggal' => $tanggal,
